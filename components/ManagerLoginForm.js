...
// /api/manager-login, which is the only thing that can actually grant
// access.
export default function ManagerLoginForm() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      // renders the real sales data - the client never fetches or
      // holds that data itself.
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-sm text-ink/60">
        Enter the shared manager password to view recent sales.
      </p>

      <label htmlFor="manager-password" className="mt-6 block text-sm font-medium text-ink">
        Password
      </label>
      <input
        id="manager-password"
