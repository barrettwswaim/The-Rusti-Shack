...
// Inventory/reorder view. Lead time and safety stock are NOT present in
// the source data (see DATA_IMPORT_REPORT.md section 6) so they are
// never invented - they're manager-editable assumptions passed in from
// the UI, with defaults of 14 lead-time days / 7 safety-stock days.
// Everything else (on-hand qty, sales+rental velocity, months of real
// history) is computed live from real data.
export async function getInventoryStatus(leadTimeDays = 14, safetyStockDays = 7) {
  ...
  if (error) throw new Error(`getInventoryStatus: ${error.message}`);
  return (data || []).map((r) => ({
    sku: r.sku,
    productName: r.product_name || r.sku,
    category: r.category,
    onHandQty: Number(r.on_hand_qty) || 0,
    sourceReorderPoint: r.source_reorder_point === null ? null : Number(r.source_reorder_point),
    saleUnitsTotal: Number(r.sale_units_total) || 0,
    rentalUnitsTotal: Number(r.rental_units_total) || 0,
    monthsOfHistory: Number(r.months_of_history) || 0,
    avgMonthlyDemand: Number(r.avg_monthly_demand) || 0,
